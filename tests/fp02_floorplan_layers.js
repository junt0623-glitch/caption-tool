...
async function run() {
  ...
  t.eq(await remainOf('p_153'), '残 27 / 27', '展示台1.53×0.9の残数は27');
  t.eq(await remainOf('p_180'), '残 24 / 24', '展示台1.8×0.9の残数は24');
  ...
  t.eq(await remainOf('c_alpha'), '残 1 / 2', '取り出すと残数が1減る');
  await page.keyboard.press('Delete');
  await page.waitForTimeout(100);
  t.eq(await remainOf('c_alpha'), '残 2 / 2', '配置したケースを削除すると残数が戻る');
  ...
  const corners = await page.$eval('svg#plan .sheet-root > image', i => {
    ...
    const deg = m ? +m[1] : 0, cx = m ? +m[2] : 0, cy = m ? +m[3] : 0;
  ...
  t.eq([Math.min(...xs), Math.max(...xs)], [0, 29700], '回転後の図面の左右端は印刷範囲(A3縦の幅29700)にちょうど一致する');
  t.eq([Math.min(...ys), Math.max(...ys)], [0, 42000], '回転後の図面の上下端は印刷範囲(A3縦の高さ42000)にちょうど一致する');
